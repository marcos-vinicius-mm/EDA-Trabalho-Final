...
public:
     ...
     * caso contrario lança uma std::out_of_range exception.
     * 
     ...
     * Atenção: cuidado com divisão inteira: você deve retornar um float.
     */
     ...
     * A função clear() de cada lista encadeada é chamada e as listas são esvaziadas, 
     ...
     * Retorna false caso a inserção não seja feita.
     ...
     * @param v := valor 
     */
    Value& insert(const Key& k, const Value& v) override {
        size_t slot = hash_code(k);
        for (auto& par : m_table[slot]) {
            m_metrics.comparisons++;
            if (par.first == k) return par.second;   // j� existe
        }
        if (needs_rehash()) {
            rehash(2 * m_table_size);
            slot = hash_code(k);
    ...
    bool remove(const Key& k){
    	size_t slot = hash_code(k);
    	for(size_t i = m_table[slot].begin(); i != m_table[slot].end(); ++i) {
            if(par.first == k) {
     ...
     * o tamanho da tabela em uma unidade (incrementa m_number_of_elements), 
     * mesmo se nenhum valor mapeado for atribuido 
